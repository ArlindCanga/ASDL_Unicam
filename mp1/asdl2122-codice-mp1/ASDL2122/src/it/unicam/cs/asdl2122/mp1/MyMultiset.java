 ...
 * di un elemento avendoci quindi un unico puntatore per ogni oggetto indipendentemente dalla sua molteplicità.
 * 
...
public class MyMultiset<E> implements Multiset<E> {
    ...
    // Come parametro ricevo un elemento da ricercare, se esiste lo ritorno
    ...
    private class Itr implements Iterator<E> {
        ...
        private int repeat; //campo con cui gestisco le occorrenze in caso di next
    ...
    public int count(Object element) {
        ...
        if (this.head != null){ // Se il multiset non è vuoto controllo che l'elemento esista
            ...
            if (nodetosearch != null){ //il campo esiste e ritorno le sue occorrenze
    ...
    // poichè andrebbe a scorrere i nodi ogni volta che devo incrementare le occurrences
    @Override
    public int add(E element, int occurrences) {
        ...
        //creo il nodo e successivamente lo concateno agli altri
        ...
        if (this.head == null){ //se il Multiset è vuoto creo il primo nodo
            ...
            //controllo se il Nodo già esiste, in questo caso aumento solo le occurrences
    ...
    public int remove(Object element, int occurrences) {
        ...
        Node<E> newNode = new Node<E>((E) element, null, occurrences);
        Node<E> nodetosearch = this.searchNode(newNode); //testo se il nodo esiste
        int ret = 0;
        if (nodetosearch != null){
            ret = nodetosearch.occurrencies; //mi segno le occurrences prima di aggiornarle poichè devo tornarle
            if ((nodetosearch.occurrencies - occurrences) <= 0){ //controllo se devo eliminare del tutto il nodo
                if (nodetosearch.item == this.head.item){ //controllo se è l'elemento head
                    ...
                    //sistemo la lista concatenata eliminando il nodo
    ...
    //questo metodo ha la stessa logica di remove(element, occurrences) dove occurrences = 1
    @Override
    ...
    public int setCount(E element, int count) {
        ...
        Node<E> nodetosearch = this.searchNode(newNode);
        int ret = 0;
        //una volta individuato il Nodo lo rimando su add o remove in base alle sue occurrences
        if (nodetosearch != null){
    ...
    public boolean contains(Object element) {
        ...
        Node<E> newNode = new Node<E>((E) element, null);
        Node<E> nodetosearch = this.searchNode(newNode); //il metodo ritorna l'oggetto se esiste, altrimenti null
    ...
    public boolean equals(Object obj) {
        ...
        //due multiset sono uguali se rispettano 3 condizioni:
        // - hanno stessa size;
        // - Stessi elementi
        // - Stesse occurrences
