...
public class EquazioneSecondoGradoModificabileConRisolutore {
    ...
    public EquazioneSecondoGradoModificabileConRisolutore(double a, double b,
        ...
        if (Math.abs(a) < EPSILON)
            throw new IllegalArgumentException("L'equazione di secondo grando non può avere coifficiente uguale a zero");
        this.a = a;
        this.b = b;
        this.c = c;
    }
    ...
    public void setA(double a) {
        if (Math.abs(a) < EPSILON) // controllo se uguale a zero
            throw new IllegalArgumentException("a non può essere 0");
        this.a = a;
        this.solved = false;
    }
     ...
     */
    public void solve() {
        if (!this.solved) { //risolvo solo se non è già stata risolta
            double delta = this.b * this.b - 4 * this.a * this.c;
            if (delta < 0) // Delta < 0 allora non ci sono soluzioni
                this.lastSolution = new SoluzioneEquazioneSecondoGrado(new EquazioneSecondoGrado(this.a, this.b, this.c));
            else {
                if (Math.abs(delta) < EPSILON) { // < 0 è stato escluso, si testa se è < di un valore poco sopra lo 0
                    ...
                    // salva la soluzione con due valori
                    this.lastSolution = new SoluzioneEquazioneSecondoGrado(
                            new EquazioneSecondoGrado(this.a, this.b, this.c),
                            (-this.b + temp) / 2 * this.a, (-this.b - temp) / 2 * this.a);
                }
            }
            this.solved = true;
        }
